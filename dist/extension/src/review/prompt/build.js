export function buildReviewPrompt(snapshot, drafts) {
    ...
    for (const file of snapshot.files) {
        ...
        const sorted = [...fileDrafts].sort((a, b) => draftSortKey(a) - draftSortKey(b));
        for (const draft of sorted) {
            ...
            const lineLabel = draft.anchor.side === "old"
...
function groupDraftsByFile(files, drafts) {
    const order = new Set(files.map((file) => file.path));
    const groups = new Map();
    for (const draft of drafts) {
        if (!order.has(draft.anchor.path))
            continue;
...
function draftSortKey(draft) {
    if (draft.anchor.side === "file")
        return -1;
    return ((draft.anchor.hunkIndex ?? 0) * 1_000_000 + (draft.anchor.rowIndex ?? 0));
}
function compactContext(file, anchorId) {
