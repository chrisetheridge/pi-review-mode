import { agentCommentsToSeedDrafts } from "../agent/coordinator.js";
...
export async function runReviewCommand({ args, ctx, pi, agentReviews }) {
    try {
        const options = parseReviewCommand(commandInput(args));
        if (!ctx.hasUI) {
            await ctx.ui?.notify?.("Review mode requires an interactive Pi UI.", "error");
            return;
        }
        const snapshot = options.fixture
            ? new FixtureReviewSource(ctx.cwd ?? process.cwd()).createSnapshot(assertFixtureModeEnabled(options.fixture))
            : await createGitSnapshot(options.base, ctx);
        if (!snapshot) {
            await ctx.ui?.notify?.("Review cancelled. Editor contents were not changed.", "info");
            return;
        }
        const config = loadReviewModeConfig();
        const shouldRunAgentReview = options.agent || (!options.fixture && config.agentReview);
        const seedDrafts = shouldRunAgentReview
            ? await collectAgentPreReview(snapshot, agentReviews, pi, ctx)
            : [];
        await ctx.ui?.notify?.("Opening native review window for a frozen Git snapshot.", "info");
        const result = await openGlimpseReviewSurface(snapshot, {
            seedDrafts,
            onSubmitPrompt: async (prompt) => {
                await ctx.ui?.setEditorText?.(prompt);
                await ctx.ui?.notify?.("Review feedback was written into the editor.", "success");
            }
        });
        if (result.prompt) {
            return;
        }
        await ctx.ui?.notify?.("Review closed. Editor contents were not changed.", "info");
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Unable to start review mode.";
...
function assertFixtureModeEnabled(fixture) {
    if (process.env.PI_REVIEW_MODE_FIXTURES !== "1") {
        throw new Error("Review fixtures are development-only. Set PI_REVIEW_MODE_FIXTURES=1 to use /review --fixture.");
    }
    return fixture;
}
async function collectAgentPreReview(snapshot, agentReviews, pi, ctx) {
    ...
    try {
        if (!pi.sendMessage || !ctx.waitForIdle) {
            await ctx.ui?.notify?.("Agent pre-review is unavailable in this Pi runtime. Opening the review window with no seeded comments.", "warning");
            return [];
        }
        pi.sendMessage({
        ...
        if (comments.length === 0) {
            await ctx.ui?.notify?.("Agent pre-review returned no comments. Opening the review window.", "warning");
        }
        else {
            await ctx.ui?.notify?.(`Agent pre-review seeded ${comments.length} comment${comments.length === 1 ? "" : "s"}.`, "info");
        }
        return agentCommentsToSeedDrafts(comments);
    }
    catch (error) {
        await ctx.ui?.notify?.(`Agent pre-review failed: ${error instanceof Error ? error.message : "Unknown error"}. Opening the review window with no seeded comments.`, "warning");
        return [];
    }
    finally {
...
async function createGitSnapshot(base, ctx) {
...
async function selectReviewScope(source, ctx) {
    ...
    const selectedLabel = await ctx.ui?.select?.("Choose changes to review", options.map((option) => option.label));
