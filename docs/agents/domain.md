...
| Review session and surface | `src/review/in-process-review-session.ts`, `src/review/glimpse-review-surface.ts` | Changing draft validation, seeded drafts, Glimpse message handling, close, submit, native window opening, or lifecycle cleanup. | PRD sections: Draft storage, Security and lifecycle, Data loading |
...
| Review UI | `apps/review-web/**` | Changing file tree, unified diff rendering, comment editors, saved comment cards, submit blocking, close behavior, transport, or styling. | PRD sections: Browser review UI, Comment behavior, Data loading |
| Backend tests | `test/review/**` | Adding or changing backend behavior for command parsing, Git fixtures, snapshots, parser, prompt builder, session, or Glimpse surface. | `CONTEXT.md` verification commands |
| Web UI tests | `apps/review-web/**/*.test.tsx` | Adding or changing UI rendering and review interaction tests. | PRD acceptance criteria for browser UI and comment behavior |
